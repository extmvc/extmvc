class Admin::<%= controller_class_name %>Controller < AdminController
  ...
  def show
    # show.html.erb
    respond_to do |format|
      format.ext_json { render :json => [@<%= file_name %>].to_ext_json(:class => :<%= file_name %>)}
    end
  end
  ...
  def create
    ...
    respond_to do |format|
      if @<%= file_name %>.save
        ...
        format.ext_json { render :json => @<%= file_name %>.to_ext_json(:success => true) }
      else
  ...
  protected
    ...
    def find_<%= table_name %>
      ...
      @<%= file_name %>s = <%= class_name %>.find(:all, options_from_pagination_state(pagination_state).merge(options_from_search(:<%= file_name %>)))
    end
