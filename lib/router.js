Ext.ux.MVC.Router = function(config) {
  ...
  this.split = function(pattern) {
    ...
    for (var i=0; i < this.mappings.length; i++) {
      ...
      re = new RegExp(this.convert_regex(m));
  ...
  this.convert_regex = function(regex_string) {
    ...
    route_params = this.params_for(regex_string);
  ...
  this.recognise = function(url) {
    ...
    keys = this.params_for(this.matching_route_for(url));
    values = this.split(url);
