...
String.prototype.toCurrency = function(symbol) {
  ...
  var segmentCount      = Math.floor(this.length / 3);
  var firstSegmentWidth = this.length % 3,
      pointerPosition   = firstSegmentWidth;
  
  var segments = firstSegmentWidth == 0 ? [] : [this.substr(0, firstSegmentWidth)];
  
  for (var i=0; i < segmentCount; i++) {
    segments.push(this.substr(firstSegmentWidth + (i * 3), 3));
  };
  
  return symbol + segments.join(",");
};
